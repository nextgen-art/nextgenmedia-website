...
function normalizeSource(ghlData) {
  ...
  // Only use string values -- some fields (e.g. attributionSource) may be objects
  const source = (
    (typeof ghlData.source === 'string' && ghlData.source) ||
    (typeof ghlData.attributionSource === 'string' && ghlData.attributionSource) ||
    attribution.utm_source || attribution.utmSource ||
    attribution.sessionSource ||
    ''
  ).toLowerCase();
  const medium = (
    (typeof ghlData.medium === 'string' && ghlData.medium) ||
  ...
  if (source === 'crm' || source === 'crm ui')                return { source: 'crm',          medium: 'manual' };
...
exports.handler = async (event) => {
  ...
  if (!clientId) {
    console.warn(`No client found for GHL location: ${locationId}. Set the GHL Location ID in the Admin Dashboard.`);
  ...
  // GHL workflow payloads use contact_id (snake_case)
...
// Create /netlify/functions/hubspot-webhook.js, etc.
// Normalize their payload into the same lead record shape.
